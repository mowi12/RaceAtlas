...
**Known Issue:** The build process requires internet access to fetch Google Fonts (Poppins font in `src/app/layout.tsx`). In sandboxed/offline environments, the build will fail with:
...
- `src/app/layout.tsx` - Root layout (uses Poppins font)
- `src/app/globals.css` - Global Tailwind styles
- `src/app/page.tsx` - Root redirect page
- `src/app/[locale]/` - Internationalized routes
    - `src/app/[locale]/page.tsx` - Home page (timeline view)
    - `src/app/[locale]/calendar/page.tsx` - Calendar view
    - `src/app/[locale]/timeline/page.tsx` - Timeline view
    - `src/app/[locale]/map/page.tsx` - Map view
    - `src/app/[locale]/events/[eventId]/page.tsx` - Event detail page

**`src/lib/`** - Reusable code (path alias: `@/lib`)

-- `src/lib/components/` - React components
    - `src/lib/components/primitives/` - Base UI components (shadcn/ui)
    - `src/lib/components/composites/` - Composed components
        - `src/lib/components/composites/navigation/` - Navigation components
        - `src/lib/components/composites/timeline/` - Timeline components
        - `src/lib/components/composites/theme/` - Theme switcher
        - `src/lib/components/composites/language-select/` - Language selector
- `src/lib/types/` - TypeScript type definitions
    - `event.ts`, `race.ts`, `geo.ts`, `route.ts`, `i18n.ts`
-- `src/lib/data/` - Data and constants
    - `src/lib/data/example-events.ts` - Example event data
-- `src/lib/filter/` - Event filtering logic
    - `src/lib/filter/filter-events.ts` - Filter implementation
    - `src/lib/filter/event-filters.ts` - Filter definitions
-- `src/lib/search/` - Event search logic
    - `src/lib/search/event-search.ts` - Fuse.js fuzzy search
-- `src/lib/sort/` - Event sorting logic
    - `src/lib/sort/sort-events.ts` - Sort implementations
-- `src/lib/geo/` - Geographic utilities
    - `src/lib/geo/distance.ts` - Distance calculations
-- `src/lib/i18n/` - Internationalization
    - `src/lib/i18n/request.ts` - i18n request handler
    - `src/lib/i18n/navigation.ts` - Localized navigation
    - `src/lib/i18n/routing.ts` - Route configuration
    - `src/lib/i18n/localized-text.ts` - Text localization utilities
-- `src/lib/store/` - State management (Zustand)
    - `src/lib/store/event-store.ts` - Event store
-- `src/lib/navigation/` - Navigation configuration
-- `src/lib/utils/` - Utility functions
    - `src/lib/utils/shadcn-helper.ts` - shadcn/ui utilities
...
-- Add translations to `src/messages/en.json` and `src/messages/de.json`

**Adding a component:**

-- Place in `src/lib/components/composites/` for app-specific components
-- Place in `src/lib/components/primitives/` for base UI components
...
- ❌ Don't modify files excluded in `biome.json`:
    - `src/lib/components/primitives/item.tsx`
    - `src/lib/components/primitives/slider.tsx`
